...
impl HttpMCPServer {
    ...
    fn handle_tool_call(&self, request_id: Option<&Value>, params: &Value) -> Result<Value> {
        ...
        eprintln!("[HTTP MCP] Tool call: {} with args: {}", tool_name, arguments);
    ...
    fn fetch_url(&self, args: &Value) -> Result<Value> {
        ...
        let safe_domains = ["httpbin.org", "jsonplaceholder.typicode.com", "api.github.com"];
    ...
    fn get_time(&self, args: &Value) -> Result<Value> {
        ...
        let time_str = match timezone.to_uppercase().as_str() {
            ...
            _ => {
                format!("Current time in {}: {} (timezone not recognized, showing UTC)", 
                       timezone, now.format("%Y-%m-%d %H:%M:%S UTC"))
            }
        };
